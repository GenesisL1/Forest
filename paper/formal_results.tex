...
\subsection{Proof obligations for a publication-grade deployment}

The strongest reproducibility record should contain:
\begin{enumerate}
  \item the exact core bytes and both SHA-256 and Keccak-256 digests;
  \item all chunk addresses, runtime code hashes, ordered table address,
        $c,N,$ and $\ell$;
  \item a strict parser report covering magic, version, reserved fields,
        dimensions, exact length, and every feature index;
  \item the nine named registry/core/storage relations above, plus any
        separately claimed NFT feature metadata;
  \item packed feature vectors and integer outputs for conformance cases;
  \item source revision, compiler version and binary digest, optimizer
        settings, IR setting, EVM target, creation bytecode, and deployed
        bytecode hash; and
  \item a parity matrix that distinguishes byte-identical training evidence
        from the inference theorem proved above.
\end{enumerate}

These obligations make the claim falsifiable: an independent party can either
reproduce every integer output or identify the first violated premise.
