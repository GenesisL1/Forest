...
candle and propagate the funding rate) and:
  ...
  4. Calls ForestRuntime.predictView(modelId, packed) on GenesisL1 (tokenId 8)
  5. Computes probability = sigmoid(rawLogit / scaleQ)
  6. Appends a row to xrpup_predictions.csv with the candle OHLC + score
...
Run:
    python3 xrpup_predictor_daemon.py
...
CSV_PATH = Path(os.environ.get(
    'XRPUP_CSV', str(SCRIPT_DIR / 'xrpup_predictions.csv')
))
...
HTML_PATH = Path(os.environ.get(
    'XRPUP_HTML', str(SCRIPT_DIR / 'xrpup_5h_scrying.html')
))

# Maximum rows to render in the history table.
HISTORY_MAX_ROWS = int(os.environ.get('XRPUP_HISTORY_ROWS', '500'))
...
def build_features(klines, funding, i=None):
    ...
    # XRP-specific (XRP uses the full feature set; ETH UP omits these)
...
def dedupe_existing_csv():
    ...
    fd, tmp_path = tempfile.mkstemp(
        prefix='.xrpup_', suffix='.csv.tmp', dir=str(CSV_PATH.parent)
    )
    try:
...
CSV_MAX_ROWS = int(os.environ.get('XRPUP_CSV_MAX_ROWS', '500'))
...
HISTORY_PAGE_SIZE = int(os.environ.get('XRPUP_PAGE_SIZE', '50'))
...
def _render_history_section(rows):
    ...
    if not rows:
        return (
            '  <!-- HISTORY:BEGIN — content below is regenerated by xrpup_predictor_daemon.py -->\n'
...
def regenerate_html(rows):
    ...
    fd, tmp_path = tempfile.mkstemp(
        prefix='.xrpup_', suffix='.html.tmp', dir=str(HTML_PATH.parent)
    )
    try:
...
KLINES_FETCH_LIMIT       = int(os.environ.get('XRPUP_KLINES_LIMIT',       '251'))
KLINES_FETCH_LIMIT_BOOT  = int(os.environ.get('XRPUP_KLINES_LIMIT_BOOT',  '500'))
...
RETRY_SAFETY_MARGIN_S = float(os.environ.get('XRPUP_RETRY_SAFETY_S', '20'))
# First retry waits this long; each subsequent wait doubles up to the cap.
RETRY_BACKOFF_INITIAL_S = float(os.environ.get('XRPUP_RETRY_BACKOFF_S', '5'))
RETRY_BACKOFF_CAP_S     = float(os.environ.get('XRPUP_RETRY_CAP_S',     '45'))
...
def main():
    ...
    if os.environ.get('XRPUP_INIT_NOW', '1') == '1':
