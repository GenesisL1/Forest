#!/usr/bin/env python3
"""
xrpdn_predictor_daemon.py
=========================

Runs every 15-minute candle close (+ 3 second buffer for Binance to settle the
candle and propagate the funding rate) and:

  1. Fetches the most recent closed 15m XRPUSDT candle + ~250 bars of context
  2. Computes the same 33 causal features the dApp computes
  3. Quantises to int32 at scaleQ, packs little-endian
  4. Calls ForestRuntime.predictView(modelId, packed) on GenesisL1 (tokenId 9)
  5. Computes probability = sigmoid(rawLogit / scaleQ)
  6. Appends a row to xrpdn_predictions.csv with the candle OHLC + score

Targets the DOWN_XRP model (NFT #9): probability of a −2.0% drawdown below
EMA5 within 5h, before any +0.7% retrace above. Symmetric counterpart to
xrpup_predictor_daemon.py (NFT #8).
...
Run:
    python3 xrpdn_predictor_daemon.py
...
CSV_PATH = Path(os.environ.get(
    'XRPDN_CSV', str(SCRIPT_DIR / 'xrpdn_predictions.csv')
))
...
HTML_PATH = Path(os.environ.get(
    'XRPDN_HTML', str(SCRIPT_DIR / 'xrpdn_5h_scrying.html')
))

# Maximum rows to render in the history table.
HISTORY_MAX_ROWS = int(os.environ.get('XRPDN_HISTORY_ROWS', '500'))
...
# Both XRP models use the full 33-feature small set + open_time.
FEATURE_ORDER = [
...
def build_features(klines, funding, i=None):
    """Mirrors the dApp's buildFeatures() for the DOWN_XRP model (33 features).
    ...
    # Multi-bar returns. NB: training names are quirky (return_5_3 = pct_change(3),
    # return_60_3 = pct_change(36) at 15m); names preserved verbatim from .gl1f schema.
    feats['return_5_3']  = (klines[i]['c'] / klines[i-3]['c']) - 1.0
    feats['return_60_3'] = (klines[i]['c'] / klines[i-36]['c']) - 1.0

    # volatility_5 = std (ddof=1) of pct-change over rolling 12 bars
    feats['volatility_5'] = roll_std(r1pct, i, 12)
    # mom_5 = sum of pct-change over rolling 5 bars
    ...
    # Signed pct distance to EMA5 baseline — same coordinate system as the label
    feats['close_to_ema5'] = c / ema5 - 1.0
    # Same distance scaled by ATR(14)
    ...
    # body_signed_3 = mean over last 3 bars of (c-o)/(h-l) — signed body fraction
    bs_sum = 0.0
...
CSV_HEADERS = [
    ...
    'target_price',          # ema5 * 0.980 (downside target, −2.0%)
    'stop_price',            # ema5 * 1.007 (upside invalidation, +0.7%)
...
def dedupe_existing_csv():
    ...
    fd, tmp_path = tempfile.mkstemp(
        prefix='.xrpdn_', suffix='.csv.tmp', dir=str(CSV_PATH.parent)
    )
    try:
...
CSV_MAX_ROWS = int(os.environ.get('XRPDN_CSV_MAX_ROWS', '500'))
...
def _fmt_dollar(s):
    ...
    if not math.isfinite(n):
        return '—'
    # XRP trades sub-$10 most of the time; 4 dp keeps EMA / target / stop legible.
    return f'${n:,.4f}'
...
HISTORY_PAGE_SIZE = int(os.environ.get('XRPDN_PAGE_SIZE', '50'))
...
def _render_history_section(rows):
    ...
    if not rows:
        return (
            '  <!-- HISTORY:BEGIN — content below is regenerated by xrpdn_predictor_daemon.py -->\n'
    ...
    return (
        ...
        '            <th class="num">Target −2.0%</th>\n'
        '            <th class="num">Stop +0.7%</th>\n'
        '          </tr>\n'
        '        </thead>\n'
        '        <tbody>\n'
        f'{tbody}\n'
...
def regenerate_html(rows):
    ...
    fd, tmp_path = tempfile.mkstemp(
        prefix='.xrpdn_', suffix='.html.tmp', dir=str(HTML_PATH.parent)
    )
    try:
...
def _log_one_candle(w3, klines, funding, idx, meta):
    ...
    # DOWN-model target/stop: −2.0% drawdown, +0.7% retrace stop
    target = ema5 * 0.980
    stop   = ema5 * 1.007
...
KLINES_FETCH_LIMIT       = int(os.environ.get('XRPDN_KLINES_LIMIT',       '251'))
KLINES_FETCH_LIMIT_BOOT  = int(os.environ.get('XRPDN_KLINES_LIMIT_BOOT',  '500'))
...
RETRY_SAFETY_MARGIN_S = float(os.environ.get('XRPDN_RETRY_SAFETY_S', '20'))
# First retry waits this long; each subsequent wait doubles up to the cap.
RETRY_BACKOFF_INITIAL_S = float(os.environ.get('XRPDN_RETRY_BACKOFF_S', '5'))
RETRY_BACKOFF_CAP_S     = float(os.environ.get('XRPDN_RETRY_CAP_S',     '45'))
...
def main():
    ...
    if os.environ.get('XRPDN_INIT_NOW', '1') == '1':
