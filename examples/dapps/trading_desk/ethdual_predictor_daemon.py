#!/usr/bin/env python3
"""ethdual_predictor_daemon.py — merge ethup2 + ethdn2 prediction CSVs into
the dual-model history block of ethdual_5h_scrying.html.

This daemon does NO inference of its own. It reads:
  - ethup2_predictions.csv  (produced by ethup2_predictor_daemon.py)
  - ethdn2_predictions.csv  (produced by ethdn2_predictor_daemon.py)
...
CSV_UP_PATH = Path(os.environ.get(
    'ETHDUAL_CSV_UP', str(SCRIPT_DIR / 'ethup2_predictions.csv')
))
CSV_DN_PATH = Path(os.environ.get(
    'ETHDUAL_CSV_DN', str(SCRIPT_DIR / 'ethdn2_predictions.csv')
))
HTML_PATH = Path(os.environ.get(
    'ETHDUAL_HTML', str(SCRIPT_DIR / 'ethdual_5h_scrying.html')
))

POLL_INTERVAL_S    = int(os.environ.get('ETHDUAL_POLL_S',    '20'))
HISTORY_MAX_ROWS   = int(os.environ.get('ETHDUAL_MAX_ROWS',  '500'))
HISTORY_PAGE_SIZE  = int(os.environ.get('ETHDUAL_PAGE_SIZE', '50'))

# Bias magnitude (as fraction in [0,1]) below which we render the bias as
# neutral rather than directional.
BIAS_NEUTRAL_BAND  = float(os.environ.get('ETHDUAL_BIAS_NEUTRAL', '0.05'))
...
def _render_empty_section() -> str:
    return (
        '  <!-- HISTORY:BEGIN — content below is regenerated by ethdual_predictor_daemon.py -->\n'
        ...
        '      Inner-join of ethup2_predictions.csv + ethdn2_predictions.csv is empty. Both source daemons need to be running and to have produced predictions for at least one shared candle.\n'
        '    </div>\n'
        '  </section>\n'
        '  <!-- HISTORY:END -->'
    )
...
def regenerate_html(merged: list[tuple[dict, dict]],
    ...
    fd, tmp_path = tempfile.mkstemp(
        prefix='.ethdual_', suffix='.html.tmp', dir=str(HTML_PATH.parent)
    )
    try:
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description='Merge ethup2 + ethdn2 prediction CSVs into the dual '
                    'page history block.'
    )
