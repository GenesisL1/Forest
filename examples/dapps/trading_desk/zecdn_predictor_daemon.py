...
candle and propagate the funding rate) and:
  ...
  6. Appends a row to zecdn_predictions.csv with the candle OHLC + score

Targets the DOWN_ZEC model (NFT #11): probability of a −4.0% drawdown below
EMA5 within 5h, before any +1.0% retrace above. Symmetric counterpart to
zecup_predictor_daemon.py (NFT #8).
...
Run:
    python3 zecdn_predictor_daemon.py
...
CSV_PATH = Path(os.environ.get(
    'ZECDN_CSV', str(SCRIPT_DIR / 'zecdn_predictions.csv')
))
...
HTML_PATH = Path(os.environ.get(
    'ZECDN_HTML', str(SCRIPT_DIR / 'zecdn_5h_scrying.html')
))

# Maximum rows to render in the history table.
HISTORY_MAX_ROWS = int(os.environ.get('ZECDN_HISTORY_ROWS', '500'))
...
def build_features(klines, funding, i=None):
    """Mirrors the dApp's buildFeatures() for the DOWN_ZEC model (33 features).
...
CSV_HEADERS = [
    ...
    'target_price',          # ema5 * 0.960 (downside target, −4.0%)
    'stop_price',            # ema5 * 1.010 (upside invalidation, +1.0%)
...
def dedupe_existing_csv():
    ...
    fd, tmp_path = tempfile.mkstemp(
        prefix='.zecdn_', suffix='.csv.tmp', dir=str(CSV_PATH.parent)
    )
    try:
...
CSV_MAX_ROWS = int(os.environ.get('ZECDN_CSV_MAX_ROWS', '500'))
...
HISTORY_PAGE_SIZE = int(os.environ.get('ZECDN_PAGE_SIZE', '50'))
...
def _render_history_section(rows):
    ...
    if not rows:
        return (
            '  <!-- HISTORY:BEGIN — content below is regenerated by zecdn_predictor_daemon.py -->\n'
...
def regenerate_html(rows):
    ...
    fd, tmp_path = tempfile.mkstemp(
        prefix='.zecdn_', suffix='.html.tmp', dir=str(HTML_PATH.parent)
    )
    try:
...
def _log_one_candle(w3, klines, funding, idx, meta):
    ...
    # DOWN-model target/stop: −4.0% drawdown, +1.0% retrace stop
    target = ema5 * 0.960
    stop   = ema5 * 1.010
...
KLINES_FETCH_LIMIT       = int(os.environ.get('ZECDN_KLINES_LIMIT',       '251'))
KLINES_FETCH_LIMIT_BOOT  = int(os.environ.get('ZECDN_KLINES_LIMIT_BOOT',  '500'))
...
RETRY_SAFETY_MARGIN_S = float(os.environ.get('ZECDN_RETRY_SAFETY_S', '20'))
# First retry waits this long; each subsequent wait doubles up to the cap.
RETRY_BACKOFF_INITIAL_S = float(os.environ.get('ZECDN_RETRY_BACKOFF_S', '5'))
RETRY_BACKOFF_CAP_S     = float(os.environ.get('ZECDN_RETRY_CAP_S',     '45'))
...
def main():
    ...
    if os.environ.get('ZECDN_INIT_NOW', '1') == '1':
