...
candle and propagate the funding rate) and:
  ...
  2. Computes the same 26 causal features the dApp computes
...
Run:
    python3 ethup2_predictor_daemon.py
...
CSV_PATH = Path(os.environ.get(
    'ETHUP2_CSV', str(SCRIPT_DIR / 'ethup2_predictions.csv')
))
...
HTML_PATH = Path(os.environ.get(
    'ETHUP2_HTML', str(SCRIPT_DIR / 'ethup2_5h_scrying.html')
))

# Maximum rows to render in the history table.
HISTORY_MAX_ROWS = int(os.environ.get('ETHUP2_HISTORY_ROWS', '500'))
...
FEATURE_ORDER = [
    ...
    'ATR_HL_ratio100', 'TrendConsist100', 'return_60_3', 'hour', 'dow',
    'funding_rate', 'funding_rate_ma_24h', 'funding_cum_24h',
    'dist_ema5_atr', 'rv6_over_rv24', 'body_signed_3', 'taker_imb_sum_6',
    'ret_skew_24h', 'sign_persist_12', 'range_efficiency_6',
    'trend_r2_12', 'dist_low24_atr'
]
...
CSV_HEADERS = [
    ...
    'target_price',          # ema5 * 1.020
    'stop_price',            # ema5 * 0.993
...
def dedupe_existing_csv():
    ...
    fd, tmp_path = tempfile.mkstemp(
        prefix='.ethup2_', suffix='.csv.tmp', dir=str(CSV_PATH.parent)
    )
    try:
...
CSV_MAX_ROWS = int(os.environ.get('ETHUP2_CSV_MAX_ROWS', '500'))
...
HISTORY_PAGE_SIZE = int(os.environ.get('ETHUP2_PAGE_SIZE', '50'))
...
def _render_history_section(rows):
    ...
    if not rows:
        return (
            '  <!-- HISTORY:BEGIN — content below is regenerated by ethup2_predictor_daemon.py -->\n'
...
def regenerate_html(rows):
    ...
    fd, tmp_path = tempfile.mkstemp(
        prefix='.ethup2_', suffix='.html.tmp', dir=str(HTML_PATH.parent)
    )
    try:
...
KLINES_FETCH_LIMIT       = int(os.environ.get('ETHUP2_KLINES_LIMIT',       '251'))
KLINES_FETCH_LIMIT_BOOT  = int(os.environ.get('ETHUP2_KLINES_LIMIT_BOOT',  '500'))
...
RETRY_SAFETY_MARGIN_S = float(os.environ.get('ETHUP2_RETRY_SAFETY_S', '20'))
# First retry waits this long; each subsequent wait doubles up to the cap.
RETRY_BACKOFF_INITIAL_S = float(os.environ.get('ETHUP2_RETRY_BACKOFF_S', '5'))
RETRY_BACKOFF_CAP_S     = float(os.environ.get('ETHUP2_RETRY_CAP_S',     '45'))
...
def main():
    ...
    if os.environ.get('ETHUP2_INIT_NOW', '1') == '1':
