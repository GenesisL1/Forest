#!/usr/bin/env python3
"""zecdual_predictor_daemon.py — merge zecup + zecdn prediction CSVs into
the dual-model history block of zecdual_5h_scrying.html.

This daemon does NO inference of its own. It reads:
  - zecup_predictions.csv  (produced by zecup_predictor_daemon.py)
  - zecdn_predictions.csv  (produced by zecdn_predictor_daemon.py)
...
CSV_UP_PATH = Path(os.environ.get(
    'ZECDUAL_CSV_UP', str(SCRIPT_DIR / 'zecup_predictions.csv')
))
CSV_DN_PATH = Path(os.environ.get(
    'ZECDUAL_CSV_DN', str(SCRIPT_DIR / 'zecdn_predictions.csv')
))
HTML_PATH = Path(os.environ.get(
    'ZECDUAL_HTML', str(SCRIPT_DIR / 'zecdual_5h_scrying.html')
))

POLL_INTERVAL_S    = int(os.environ.get('ZECDUAL_POLL_S',    '20'))
HISTORY_MAX_ROWS   = int(os.environ.get('ZECDUAL_MAX_ROWS',  '500'))
HISTORY_PAGE_SIZE  = int(os.environ.get('ZECDUAL_PAGE_SIZE', '50'))

# Bias magnitude (as fraction in [0,1]) below which we render the bias as
# neutral rather than directional.
BIAS_NEUTRAL_BAND  = float(os.environ.get('ZECDUAL_BIAS_NEUTRAL', '0.05'))
...
def _fmt_dollar(s) -> str:
    ...
    if not math.isfinite(n):
        return '—'
    # ZEC trades $40–$90 range; 2 dp matches typical futures display precision.
    return f'${n:,.2f}'
...
def _render_empty_section() -> str:
    return (
        '  <!-- HISTORY:BEGIN — content below is regenerated by zecdual_predictor_daemon.py -->\n'
        ...
        '      Inner-join of zecup_predictions.csv + zecdn_predictions.csv is empty. Both source daemons need to be running and to have produced predictions for at least one shared candle.\n'
        '    </div>\n'
        '  </section>\n'
        '  <!-- HISTORY:END -->'
    )
...
def regenerate_html(merged: list[tuple[dict, dict]],
    ...
    fd, tmp_path = tempfile.mkstemp(
        prefix='.zecdual_', suffix='.html.tmp', dir=str(HTML_PATH.parent)
    )
    try:
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description='Merge zecup + zecdn prediction CSVs into the dual '
                    'page history block.'
    )
