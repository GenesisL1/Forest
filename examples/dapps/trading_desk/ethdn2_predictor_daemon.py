#!/usr/bin/env python3
"""
ethdn2_predictor_daemon.py
==========================

Runs every 15-minute candle close (+ 3 second buffer for Binance to settle the
candle and propagate the funding rate) and:

  1. Fetches the most recent closed 15m ETHUSDT candle + ~250 bars of context
  2. Computes the same 27 causal features the dApp computes
  3. Quantises to int32 at scaleQ, packs little-endian
  4. Calls ForestRuntime.predictView(modelId, packed) on GenesisL1
  5. Computes probability = sigmoid(rawLogit / scaleQ)
  6. Appends a row to predictions.csv with the candle OHLC + score

Targets the DOWN_ETH model (NFT #7): probability of a −2.0% drawdown below
EMA5 within 5h, before any +0.7% retrace above. Symmetric counterpart to
ethup2_predictor_daemon.py.
...
Run:
    python3 ethdn2_predictor_daemon.py
...
CSV_PATH = Path(os.environ.get(
    'ETHDN2_CSV', str(SCRIPT_DIR / 'ethdn2_predictions.csv')
))
...
HTML_PATH = Path(os.environ.get(
    'ETHDN2_HTML', str(SCRIPT_DIR / 'ethdn2_5h_scrying.html')
))

# Maximum rows to render in the history table.
HISTORY_MAX_ROWS = int(os.environ.get('ETHDN2_HISTORY_ROWS', '500'))
...
def build_features(klines, funding, i=None):
    """Mirrors the dApp's buildFeatures() for the DOWN_ETH model (27 features).
    ...
    # Log returns (used for r1_log, r12_log, ret_skew_24h, rv6_over_rv24)
    r1 = [0.0] * n
    for j in range(1, n):
...
def dedupe_existing_csv():
    ...
    fd, tmp_path = tempfile.mkstemp(
        prefix='.ethdn2_', suffix='.csv.tmp', dir=str(CSV_PATH.parent)
    )
    try:
...
CSV_MAX_ROWS = int(os.environ.get('ETHDN2_CSV_MAX_ROWS', '500'))
...
HISTORY_PAGE_SIZE = int(os.environ.get('ETHDN2_PAGE_SIZE', '50'))
...
def _render_history_section(rows):
    ...
    if not rows:
        return (
            '  <!-- HISTORY:BEGIN — content below is regenerated by ethdn2_predictor_daemon.py -->\n'
...
def regenerate_html(rows):
    ...
    fd, tmp_path = tempfile.mkstemp(
        prefix='.ethdn2_', suffix='.html.tmp', dir=str(HTML_PATH.parent)
    )
    try:
...
KLINES_FETCH_LIMIT       = int(os.environ.get('ETHDN2_KLINES_LIMIT',       '251'))
KLINES_FETCH_LIMIT_BOOT  = int(os.environ.get('ETHDN2_KLINES_LIMIT_BOOT',  '500'))
...
RETRY_SAFETY_MARGIN_S = float(os.environ.get('ETHDN2_RETRY_SAFETY_S', '20'))
# First retry waits this long; each subsequent wait doubles up to the cap.
RETRY_BACKOFF_INITIAL_S = float(os.environ.get('ETHDN2_RETRY_BACKOFF_S', '5'))
RETRY_BACKOFF_CAP_S     = float(os.environ.get('ETHDN2_RETRY_CAP_S',     '45'))
...
def main():
    ...
    if os.environ.get('ETHDN2_INIT_NOW', '1') == '1':
